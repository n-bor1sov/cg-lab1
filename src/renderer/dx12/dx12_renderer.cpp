...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC* sampler_descriptors, UINT num_sampler_descriptors)
{
	...
	CD3DX12_ROOT_PARAMETER1 root_parameters[1];
 	...
 	rs_desccriptor.Init_1_1(_countof(root_parameters), root_parameters, num_sampler_descriptors, sampler_descriptors, rf_flags);
 	ComPtr<ID3DBlob> signature;
 	ComPtr<ID3DBlob> error;
 
 	HRESULT res = D3DX12SerializeVersionedRootSignature(&rs_desccriptor, rs_feature_data.HighestVersion, &signature, &error);
 	if (FAILED(res)) {
